...
"""Load INI configuration for COTBridge."""
...
def default_config_path() -> Path:
    return Path(os.environ.get("COTBRIDGE_CONFIG", "/etc/cotbridge.ini"))
...
def validate_lane_udp_binds(lanes: tuple[LaneSpec, ...]) -> None:
    ...
    for ln in lanes:
        for side, endpoint in lane_udp_bind_endpoints(ln):
            if side == "ingress":
                url_key = (
                    ln.merged.get("ingress_cot_url")
                    or ln.merged.get("INGRESS_COT_URL")
                    or ""
                )
            else:
                url_key = (
                    ln.merged.get("egress_cot_url")
                    or ln.merged.get("EGRESS_COT_URL")
                    or ""
                )
