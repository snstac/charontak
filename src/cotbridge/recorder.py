...
cotbridge is the one point every gateway routes through, so recording here
covers ADS-B, AIS, ACARS, APRS, Remote ID and SAPIENT at once, with no
per-gateway work.

Design constraints, in the order they mattered
----------------------------------------------
**cotbridge must not die because recording failed.** It is the process whose
...
Recordings live under /var/lib/cotbridge/tracks -- deliberately NOT under the
...
DEFAULT_DIR = "/var/lib/cotbridge/tracks"
...
class RecorderConfig:
    """Recorder settings, read from a cotbridge config section."""
...
class TrackRecorder:
    ...
    def __init__(
        self, config: Optional[Dict[str, Any]] = None, now: Optional[float] = None
    ) -> None:
        ...
        self._last_seen: Dict[
            str,
            Tuple[
                float, Optional[float], Optional[float], Optional[float], Optional[str]
            ],
        ] = {}
    ...
    def _compress(self, payload: bytes) -> bytes:
        ...
        return zstandard.ZstdCompressor(level=self.config.compress_level).compress(
            payload
        )
        ...
        if usage.free <= self.config.reserve_bytes:
            ...
            try:
                usage = shutil.disk_usage(
                    self._existing_ancestor(self.config.directory)
                )
            except OSError:
                pass
